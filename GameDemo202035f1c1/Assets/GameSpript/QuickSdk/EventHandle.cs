...
public class EventHandle : QuickSDKListener
{
	...
	public static void CheckCreateRole( )
	{
		...
		string zhanghao = Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("ZhangHaoID", "ID", Game_PublicClassVar.Get_wwwSet.RoseID, "RoseData");
		...
		if (PlayerPrefs.GetInt(roleKey) != 1)
		{
			...
			EventHandle.SendCreateRole();
		}
		//EventHandle.SendEnterGame( );
	}

	public static void SendCreateRole( )
	{
		Debug.Log("SendCreateRole(): " ) ;
		EventHandle eventHandle = GameObject.Find("WWW_Set").GetComponent<EventHandle>();
		eventHandle.onCreatRole( );
	}
	...
	public override void onInitSuccess()
	{
		...
		Game_PublicClassVar.Get_wwwSet.GetComponent<GetSignature>().SetChannelID(channelid);
		showLog("unityonInitSuccess[channelid]: " , channelid);
