...
public class UI_ItemMakeLearnListObj : MonoBehaviour
{
    ...
    void showPetListProperty() { 
		...
		*/
        //获取自身是否被选中
		if (Obj_FuJiObj.GetComponent<UI_ItmeMakeLearn>().nowSeclect != PetOnlyID) {
            //取消选中状态
            Obj_XuanZhong.SetActive(false);
            Obj_FuJiObj.GetComponent<UI_ItmeMakeLearn>().makeItemlasterObj = this.gameObject;
        }
    ...
    public void Click_XuanZhong() {
		...
		Obj_FuJiObj.GetComponent<UI_ItmeMakeLearn>().nowSeclect = PetOnlyID;
        Obj_FuJiObj.GetComponent<UI_ItmeMakeLearn>().UpdateShowStatus = true;
        //显示选中状态
