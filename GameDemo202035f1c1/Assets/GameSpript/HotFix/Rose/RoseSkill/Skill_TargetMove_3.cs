...
public class Skill_TargetMove_3 : MonoBehaviour
{
	...
	void Start () {
        ...
        {
            //被动技能才会设置目标位置
            string nowSkillType = Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("SkillType", "ID", this.gameObject.GetComponent<SkillObjBase>().SkillID, "Skill_Template");

            if (nowSkillType == "2"||this.GetComponent<SkillObjBase>().RoseSkillZhiShiStatus==false) {
                //设置一定方向及位置
                //Debug.Log("thisGame111 = " + this.gameObject.transform.position);
                this.gameObject.transform.position = Game_PublicClassVar.Get_game_PositionVar.Obj_Rose.transform.position;
                //Debug.Log("thisGame222 = " + this.gameObject.transform.position);
                Transform hitPosition = SkillActObj.GetComponent<AI_1>().BoneSet.transform.Find("Center").transform;        //假设命中怪物,后期如果怪物打人需要此处调整
                hitPosition.position = new Vector3(hitPosition.position.x, Game_PublicClassVar.Get_game_PositionVar.Obj_Rose.transform.position.y, hitPosition.position.z);
        ...
        if (SkillPar == "1")
        {
            ...
            {
                Vector3 vec3 = new Vector3(this.gameObject.GetComponent<SkillObjBase>().SkillTargetObj.transform.position.x, this.gameObject.GetComponent<SkillObjBase>().SkillTargetObj.transform.position.y, this.gameObject.GetComponent<SkillObjBase>().SkillTargetObj.transform.position.z);
            ...
            if (ai_property != null)
            {
                ...
                if (ai != null) {
                    ...
                    if(ai.HitStatus_LuanShe == false)
                    {
                        ...
                        //特殊受击 目前散射同一波次只能承受一次伤害
                        string skillParentStr = Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("SkillParent", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template");
                        if (skillParentStr == "7") {
                            ai.HitStatus_LuanShe = true;
                        }
                    }
                }
            }
