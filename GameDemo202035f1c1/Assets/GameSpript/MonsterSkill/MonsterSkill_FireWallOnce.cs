...
public class MonsterSkill_FireWallOnce : MonoBehaviour
{
    ...
    public float SkillTimeSum;             //技能时间累计值，用来在时间到时清空怪物列表
	...
	void OnTriggerEnter (Collider collider){
        ...
        if (collider.gameObject.layer == 14 || collider.gameObject.layer == 18)
        {
            if (collider.gameObject != null)
            {
                ...
                if (collider.gameObject != null)
                {
                    //Debug.Log("正确添加collider.gameObject = " + collider.gameObject.name);
        ...
        if (DamgeFrequencySum > DamgeFrequency)
        {
            ...
            foreach (GameObject nowObj in colliderList) {
                ...
                if (AI_Collider != this.gameObject&& AI_Collider!=null)
                {
                    ...
                    {
                    //Debug.Log("cccccccccccccccccccccccccccccccccccAI_Collider = " + AI_Collider.name);
