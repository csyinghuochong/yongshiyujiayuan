...
//嘲讽技能
public class RoseSkill_ChaoFeng_4 : MonoBehaviour
{
	...
	void Start () {
        ...
        //获取目标
        GameObject targetObj = Game_PublicClassVar.Get_game_PositionVar.Obj_Rose.GetComponent<Rose_Status>().Obj_ActTarget;
        if (targetObj != null) {
            if (targetObj.GetComponent<AI_1>() != null) {
                //获取自己出战的宠物
                GameObject[] zhaohuanList = Game_PublicClassVar.Get_function_Rose.GetRoseZhaoHuanFightObjList();
                for (int i = 0; i < zhaohuanList.Length; i++) {
                    if (zhaohuanList[i] != null) {
                        if (zhaohuanList[i].GetComponent<AIPet>() != null) {
                            if (zhaohuanList[i].GetComponent<AIPet>().AI_Status != "5") {
                                targetObj.GetComponent<AI_1>().AI_Target = zhaohuanList[i];
                            }
                        }
                    }
                }
