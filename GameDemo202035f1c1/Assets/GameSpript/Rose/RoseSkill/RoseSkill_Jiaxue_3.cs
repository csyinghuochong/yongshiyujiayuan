...
//药物加血技能
public class RoseSkill_Jiaxue_3 : MonoBehaviour
{
	...
	void Start () {
        ...
        float nowactDamgePro = float.Parse(Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("ActDamge", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template"));
        int addroseHpValue = startAddHpValue + (int)(rose_Proprety.Rose_Hp * nowactDamgePro);
        Game_PublicClassVar.Get_function_Rose.addRoseHp(addroseHpValue, "2");
        ...
        //Debug.Log("加血3触发BUFF：" + buffID  + "  this.GetComponent<SkillObjBase>().SkillID = " + this.GetComponent<SkillObjBase>().SkillID);
        if (buffID != "0" && buffID != "")
        {
        ...
        //宠物加血
        //获取当前出战宠物
        GameObject RoseChuZhanPet_NowObj = Game_PublicClassVar.Get_function_Rose.GetRosePetFightObj();
        //触发技能Buff
        if (RoseChuZhanPet_NowObj != null)
        {
            //获取治疗值
            startAddHpValue = int.Parse(Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("DamgeValue", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template"));
            float actDamgePro = float.Parse(Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("ActDamge", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template"));
            //Game_PublicClassVar.Get_function_AI.PetAddFightHp(RoseChuZhanPet_NowObj,startAddHpValue);
            Game_PublicClassVar.Get_function_AI.AI_addHp(RoseChuZhanPet_NowObj, startAddHpValue, actDamgePro);
            //获取特效名称
            effectName = Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("HitEffectName", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template");
            ...
            if (effectName != "0")
            {
                ...
                effect.SetActive(false);
                effect.transform.parent = this.transform;
                effect.transform.position = RoseChuZhanPet_NowObj.transform.position;
                effect.SetActive(true);
            }


            buffID = Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("BuffID", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template");
            if (buffID != "0" && buffID != "")
            {
                //获取当前出战宠物
                Game_PublicClassVar.Get_function_Skill.SkillBuff(buffID, RoseChuZhanPet_NowObj);
            }
        ...
        //宠物加血
        //获取当前出战宠物
        GameObject[] chuzhanListObj = Game_PublicClassVar.Get_function_Rose.GetRoseZhaoHuanFightObjList();

        for (int i = 0; i < chuzhanListObj.Length; i++) {

            RoseChuZhanPet_NowObj = chuzhanListObj[i];
            //触发技能Buff
            if (RoseChuZhanPet_NowObj != null)
            {
                //获取治疗值
