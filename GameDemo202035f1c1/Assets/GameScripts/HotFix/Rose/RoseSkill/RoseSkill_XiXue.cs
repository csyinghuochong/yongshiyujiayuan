...
public class RoseSkill_XiXue : MonoBehaviour {
	...
	void Start () {
        //Debug.Log("触发技能:" + this.gameObject.GetComponent<SkillObjBase>().SkillID);
        //获取是否播放受击特效
	...
	void Update () {
        ...
        if (SkillEffectDistance != 0)
        {
            Debug.Log("move111：" + this.gameObject.transform.position);
            transform.Translate(Vector3.forward * SkillEffectDistance * Time.deltaTime);
            Debug.Log("move222：" + this.gameObject.transform.position);
        }

        collockStatusTime = collockStatusTime + Time.deltaTime;
        if (collockStatusTime >= 1) {
            collockStatusTime = 0;
            collockList.Clear();
        }
    }
	...
	void OnTriggerEnter (Collider collider){
        ...
        //Debug.Log("旋风斩第一次碰撞调用...."+ collider.name);
