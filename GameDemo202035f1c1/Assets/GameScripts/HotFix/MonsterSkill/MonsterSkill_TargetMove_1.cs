...
public class MonsterSkill_TargetMove_1 : MonoBehaviour
{
	...
	void Start () {
        ...
        if (ParameterValue != "0" && ParameterValue != "" && ParameterValue != "-1")
        {
                ...
                switch (parValue[0])
                {
                    ...
                    case "1":
                        if (parValue.Length > 2)
                        {
                            ...
                            Debug.Log(" parValue[1] = " + parValue[1]);
                            if (startVec3.Length > 2 && endVec3.Length > 2)
                            {
            ...
            //Debug.Log("this.gameObject.transform = " + this.gameObject.transform.position + "; Vec3_End = " + Vec3_End + "旋转:"+ this.transform.localRotation.ToEuler());
        ...
        {
            //设置一定方向及位置
            Transform hitPosition = null;
            string targetObjType = Game_PublicClassVar.Get_function_Skill.returnObjType(SkillActObj);
            if (targetObjType == "1") {
                hitPosition = SkillActObj.GetComponent<Rose_Bone>().BoneSet.transform.Find("Center").transform;        //假设命中怪物,后期如果怪物打人需要此处调整
            }
            if (targetObjType == "2")
            {
	...
	void Update () {
        ...
        if (SkillActObj != null&& this.gameObject.GetComponent<SkillObjBase>().SkillID== "62000002")
        {
            this.gameObject.transform.LookAt(SkillActObj.transform);
        }
	...
	void OnTriggerEnter (Collider collider){
        ...
        {
            //Debug.Log("碰撞到的技能宠物碰撞到的技能宠物碰撞到的技能宠物");
