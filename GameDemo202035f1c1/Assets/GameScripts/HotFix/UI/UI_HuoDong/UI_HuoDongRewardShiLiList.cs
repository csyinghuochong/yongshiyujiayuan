...
public class UI_HuoDongRewardShiLiList : MonoBehaviour
{
    ...
    public void updataDengLuReward() {
        ...
        //显示标题
        string langStr = LanguageManager.Instance.LoadLocalization("需要战力达到");
    ...
    public void Btn_RewardLv()
    {
        ...
        if (roseShiLi < 1000)
        {
            string langStrHint = LanguageManager.Instance.LoadLocalizationHint("hint_344");
        ...
        if (RewardNum_Now >= RewardNum_Sum) {
            string langStrHint = LanguageManager.Instance.LoadLocalizationHint("hint_345");
            Game_PublicClassVar.Get_function_UI.GameGirdHint_Front(langStrHint);
