...
public class Demo : MonoBehaviour {
	...
	void OnGUI ()
	{
      ...
      btnTop += btnHeight + 20 * scale;
        if (GUI.Button(new Rect((Screen.width - btnWidth2) / 2, btnTop, btnWidth2, btnHeight), "isClientValid"))
      {
#if UNITY_ANDROID
         ssdk.isClientValidForAndroid(PlatformType.QQ);
#elif UNITY_IPHONE
         Debug.Log("Client is install: " + ssdk.IsClientValid(PlatformType.Littleredbook));
#endif

      }

	  btnTop += btnHeight + 20 * scale;
        if (GUI.Button(new Rect((Screen.width - btnWidth2) / 2, btnTop, btnWidth, btnHeight), "ShareLittleRedBook"))
      {
#if UNITY_ANDROID
			ShareContent content = new ShareContent();
			content.SetImageUrl("http://ww3.sinaimg.cn/mw690/be159dedgw1evgxdt9h3fj218g0xctod.jpg");
			...
			content.SetDesc("Share Image to xiaohongshu.");
			content.SetImageUrl("http://ww3.sinaimg.cn/mw690/be159dedgw1evgxdt9h3fj218g0xctod.jpg");
			content.SetShareType(ContentType.Image);
			ShareContent shareContent = new ShareContent();
			shareContent.SetShareContentCustomize(PlatformType.Littleredbook, content);
			ssdk.ShareContent(PlatformType.Littleredbook, shareContent);
#endif
      }

	  btnTop += btnHeight + 20 * scale;
        if (GUI.Button(new Rect((Screen.width - btnWidth2) / 2, btnTop, btnWidth, btnHeight), "ShareDouyinVideo -Android Only"))
      {
#if UNITY_ANDROID
			ShareContent content = new ShareContent();
			content.SetStringArrayParam("videoArray", new string[] { "/sdcard/Android/data/cn.sharesdk.demo/video.mp4","/sdcard/Android/data/cn.sharesdk.demo/video.mp4" });
