...
public class PetSkill_ZhongJi_1 : MonoBehaviour
{
	...
	void Start () {
        ...
        if (actObj != null) {
            if (actObj.GetComponent<AI_1>() != null) {
                ...
                if (effectName != "0")
                {
                    actObj.GetComponent<AI_1>().HitEffect = (GameObject)ResourcesManager.Instance.LoadEffectSync<GameObject>("Skill/" + effectName);
                    actObj.GetComponent<AI_1>().IfHitEffect = true;
                }
            }

            if (actObj.GetComponent<AIPet>() != null)
            {
                actObj.GetComponent<AIPet>().HitStatus = true;      //播放受击特效
                                                                   //获取特效名称
                string effectName = Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("HitEffectName", "ID", this.GetComponent<SkillObjBase>().SkillID, "Skill_Template");
                if (effectName != "0")
                {
                    actObj.GetComponent<AIPet>().HitEffect = (GameObject)ResourcesManager.Instance.LoadEffectSync<GameObject>("Skill/" + effectName);
        ...
        if (buffID != "0" && buffID != "")
        {
            ...
            Game_PublicClassVar.Get_function_Skill.SkillBuff(buffID, this.GetComponent<SkillObjBase>().MonsterSkillObj.GetComponent<AIPet>().AI_Target);
        ...
        {
            //Debug.Log("触发BUFF:" + buffID);
            Game_PublicClassVar.Get_function_Skill.SkillBuff(buffID, this.GetComponent<SkillObjBase>().MonsterSkillObj);
            //Game_PublicClassVar.Get_function_Skill.SkillBuff(buffID, game_PositionVar.Obj_Rose.GetComponent<Rose_Status>().Obj_ActTarget);
        }
