...
namespace CodeStage.AntiCheat.ObscuredTypes
{
	...
	public sealed class ObscuredString : IComparable<ObscuredString>, IComparable<string>, IComparable
	{
		...
		class CheckObString
		{
			...
			public CheckObString()
			{
				CheckString = cryptoKey111.ToString();
			}
		}
		...
		private CheckObString checkObString;
		...
		{
            cryptoKey = cryptoKey111.ToString();
            currentCryptoKey = cryptoKey;
			hiddenValue = InternalEncrypt(value);

			checkInit = currentCryptoKey;
			checkObString = new CheckObString();
#if UNITY_EDITOR
			fakeValue = value;
		...
		private string InternalDecrypt()
		{
			...
			if(inited == false && checkInit == null && checkObString == null)
			{
				currentCryptoKey = cryptoKey;
